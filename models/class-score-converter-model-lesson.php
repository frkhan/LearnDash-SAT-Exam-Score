...
class ModelLessonScore {
    ...
    */
    public function __construct() {
        ...
        global $lesson_formula_post_type;
        ...
        $this->post_type =  $lesson_formula_post_type;
       ...
       // $this->template_parser = $ld_sat_template_loader;
        ...
        add_action( 'p2p_init', array( $this, 'join_formulae_to_lessons' ) );
    ...
    */
    public function create_score_converter_post_type() {
        ...
        $params['singular_post_name'] = __('Lesson Score Converter',$this->post_type);
        $params['plural_post_name'] = __('LD Lesson Scorer',$this->post_type);
        $params['description'] = __('Raw lesson score to Scaled Score Conversion Charts',$this->post_type);
    ...
    */
    public function display_score_converter_meta_boxes() {
        ...
        $this->totalQuestions = get_post_meta( $post->ID, 'total-questions', true );
        $this->minValue = get_post_meta( $post->ID, 'min-score', true );
        $this->maxValue = get_post_meta( $post->ID, 'max-score', true );


        // Update global values
        $total_raw_questions = $this->totalQuestions;
        $min_scored_value = $this->minValue;
        $max_scored_value = $this->maxValue;

        $totalQuestionRows =  ceil(($this->totalQuestions+1)/2);
    ...
    */
    public function save_score_converter_meta_data() {
        ...
        if ( $this->post_type == $_POST['post_type'] && current_user_can('edit_post', $post->ID ) ) {
            ...
            $this->totalQuestions =  (isset( $_POST['total_raw_questions'] ) ? (int) esc_attr( trim($_POST['total_raw_questions'] )) : 52);
            $this->minValue =  (isset( $_POST['minimum_scaled_score'] ) ? (int) esc_attr( trim($_POST['minimum_scaled_score'] )) : 10);
            $this->maxValue =  (isset( $_POST['maximum_scaled_score'] ) ? (int) esc_attr( trim($_POST['maximum_scaled_score'] )) : 40);
            ...
            if ( !empty( $this->error_message ) ) {
                ...
                update_post_meta($post->ID,  'total-questions',  $this->totalQuestions );
                update_post_meta($post->ID,  'min-score',  $this->minValue);
                update_post_meta($post->ID,  'max-score',  $this->maxValue);
 ...
 * @return -
*/
    public function join_formulae_to_lessons() {

        p2p_register_connection_type( array(
            'name'  => 'formulae_to_lessons',
            'from'  => $this->post_type,
            'to'    => 'sfwd-lessons',
